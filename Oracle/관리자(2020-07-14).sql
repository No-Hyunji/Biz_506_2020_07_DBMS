...
/*
    여러줄 주석
    SQL 명령을 입력 할 때 명령이 끝났다 라는 것을 알리기 위해 ; 을 붙여준다. 자바처럼~
...
-- 오라클의 tab table은 현재 접속 된 사용자가 관리하는 
-- DB Object(객체)들의 정보를 보관하고 있는 table
SELECT *
FROM tab;

-- ALL_ALL_TABLES(소문자 OK) : 오라클 system 데이터 사전의 자세한 정보를 보관하는 table
SELECT *
FROM ALL_ALL_TABLES;

-- SELECT키워드는 FROM 절을 포함하는 명령문 형태로 작성을 하며
-- DBMS가 보관하고 있는 데이터를 table형식으로 보여달라 라는 명령
-- DBMS의 DML(Database Manuplation Lang) 중에서 Read(조회)를 수행하는 명령
-- CRUD 중에서 R : Read, Retrive를 실행하는 명령
SELECT *
FROM 주소록;

-- SQL 명령문을 통해서 DB 객체를 만들고 삭제하고, 데이터를 추가, 변경, 삭제를 수행할텐데
-- sys 사용자로 접속을 하게 되면 중요한 정보를 잘못 삭제, 변경 할 우려가 있기 때문에
-- 실습을 위해서 사용자를 생성하여 수행을 할 것이다.

-- 사용자를 추가하는 순서
-- 1. table Space : 데이터를 저장 할 물리적 공간을 설정
-- 2. user : 사용자를 생성하고, 물리적 저장공간과 연결

-- TableSpace생성 (Create)
-- TableSpace는 오라클에서 Data를 저장 할 물리적 공간을 설정 하는 것.
-- myTS : 앞으로 SQL을 통해서 사용 할 TableSpace의 Alias(이름)
-- '.../myTS.dbf' : 저장 할 파일 이름.
-- Size : 오라클에서는 성능의 효율성을 주기 위해 일단 빈 공간을 일정부분 설정하게 된다.
--      크기는 최초에 저장 할 데이터의 크기 등을 계산하여 설계하고 설정한다.
--      너무 작으면 효율성이 떨어지고, 너무 크면 불필요한 공간을 낭비한다.
--      오라클 xe(Express Edition)에서는 table Space의 최대 크기를 11G로 제한한다.
--      만약 Size를 10G로 지정하고, 용량이 초과되어 AUTO NEXT로 초가가 되는 경우
--      전체 Size가 11G를 넘어서면 오류가 나면서 더이상 데이터를 저장할 수 없게 된다.
-- AUTO... NEXT : 만약 초기에 지정 한 Size공간에 데이터가 가득차면
...
-- 질의작성기에서 코드를 작성 할 때 약속
-- DBMS의 SQL문은 특별한 일부 경우를 제외하고 대소문자 구별을 하지 않는다.
-- DBMS, SQL, 오라클과 관련 된 키워드는 모두 대문자로 작성 할 것
-- 변수, 값, 내용 등은 소문자로 사용하며, 특별히 대소문자를 구분해야 하는 경우는 별도로 공지

-- DROP : DDL명령의 CREATE와 반대되는 개념의 명령문
-- DROP 명령은 데이터를 물리적으로 완전삭제하는 개념이므로 매우 신중하게 사용 해야 한다.
...
-- 위에서 생성 한 TableSpace를 관리하며, 데이터를 조작 할 사용자를 생성
CREATE USER user1 IDENTIFIED BY 1234 -- 사용자 ID를 user1으로 설정하고, 초기 비번을 1234로 설정하라.
DEFAULT TABLESPACE myTS;

-- DCL : Data Control Lang.
-- 새로 생성 된 user1에게 권한을 부여하기
-- user1이 로그인만 할 수 있도록 권한(역할)을 부여하라
GRANT CONNECT TO user1;
-- user1이 로그인만 할 수 있는 권한을 제거
REVOKE CONNECT FROM user1;

-- user1이 로그인을 수행 하고 최소한으로 데이터들을 관리 할 수 있도록 권한을 부여
-- RESOURCE : 오라클에서 USER에게 줄 수 있는 권한 중 상당히 많은 일을 수행할 수 있는 권한
-- 현재 시스템에 설치 된 모든 TableSpace를 대상으로 무제한(TableSpace가 허용하는 범위) 저장이 가능 한 권한.
-- RESOURCE 권한은 Standard, Enterprice DBMS에서는 함부로 부여해서는 안된다. 
-- CONNECT와 RESOURCE 권한을 부여하게 되면 거의 DBA수준의 권한을 갖게 된다.
GRANT CONNECT,RESOURCE TO user1;

-- login권한과 table을 생성 할 수 있는 권한
GRANT CONNECT, CREATE TABLE TO user1;
-- login권한, table생성권한, 학생정보 테이블에 데이터를 추가 할 수 있는 권한
...
-- DBA권한(Roll)은 SysDBA보다 한 단계 낮은 권한 등급을 가지며,
