...
-- Recored의 제한(전체 데이터 개수 > 조건에 맞는 데이터 개수)
-- WHERE 절에 조건을 설정하여 데이터를 조회
-- PK, UNIQUE로 설정된 칼럼을 단일 조건으로 SELETION할 떄는 
-- 이 데이터는 1개의 객체로 취급한다.(VO에 담는다)
SELECT * FROM tbl_student
WHERE st_num = '20001';

-- st_num에 저장된 값이 '20010'이상이고(AND) '20010'이하인 조건에 맞는
-- 데이터 리스트(Record)만 보여달라
-- PK, UNIQUE로 설정된 칼럼을 복합 조건절로 SELECTION할 때 는
...
-- SELECTION을 수행 했을 때 나타나는 레코드가 1개 일지라도
-- 이 데이터는 리스트로 취급한다.
SELECT * FROM tbl_student 
WHERE st_name = '독고예준' ;
...
-- SELECTION & PROJENTION
-- WHERE 조건절을 사용해서 레코드를 제한하고,
-- 칼럼명을 나열하여 필요한 항목(필드)만 보고자 할 때 칼럼리스트를 제한하기
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_name = '남동예';
...
-- 비교연산자(=)와 관계연산자 OR를 조합하여 SELECTION 구분하기
SELECT st_num, st_name, st_dept, st_grade 
FROM tbl_student
WHERE st_num = '20001' OR st_num = '20003' OR st_num = '20006';

-- 한 개의 칼럼에 다수의 OR을 조건을 부여하여 
-- SELECITON을 수행 할 때는 IN 연산자를 사용하여 구현할 수 있다.
SELECT st_num, st_name, st_dept, st_grade 
FROM tbl_student
WHERE st_num IN ('20001', '20003', '20006');

-- 문자열(VARCHAR2, nVARCHAR2) 칼럼일 경우
-- 문자열 전체가 아닌 일부 부분만 일치하는 조건을 부여하여 SELECTION구현하기
