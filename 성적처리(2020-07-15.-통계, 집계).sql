...
-- tbl_student에 저장된 전체 레코드가 몇 개 인가?
...
-- tbl_student의 st_grade칼럼에 저장된 값 중에서 
-- 최대값(가장 큰값), 최소값(가장 작은 값)을 보여달라
...
-- tbl_student 데이터 중에서 학과가 어떠한 학과들이 있는지 보고싶다.
-- 학과 이름이 같은 이름들이 다수 존재 하는데, 이들 이름을 1개씩만 추출하여
...
-- 데이터를 학과 이름으로 그룹을 짓고, 각 학과에 소속된 학생들이 몇명씩인가?

-- st_dept 칼럼으로 그룹을 만들고
-- 그 그룹에 속하는 레코드가 몇개인가를 카운트 하여 보여주는 것
SELECT st_dept, COUNT(*) FROM tbl_student
GROUP BY st_dept;

-- 학년별로 소속된 학생이 몇 명인가? 알고싶어
-- st_grade 칼럼으로 그룹을 만들고,
-- 각 그룹에 속하는 Record가 몇 개 인가를 Count하여 보여준다.
...
-- 단, 나이가 가장 많은 학생의 이름, 또는 가장 나이가 가장 적은 학생의 이름을
-- 찾는데는 이 SQL로는 어렵다^^ 

SELECT st_dept, MAX(st_age), MIN(st_age) From tbl_student
group by st_dept;

-- SQL 명령문 : Query(질의)문 이라고도 부른다.
